...
The Finance Insight Assistant is a project designed to provide users with quick and accurate answers to their questions related to public companies listed on NASDAQ. It combines a **hybrid RAG pipeline** (dense vector search via ChromaDB + BM25 keyword search via Elasticsearch, fused with Reciprocal Rank Fusion) over a corpus of financial PDFs with live market data tools (Yahoo Finance), orchestrated by a LangGraph ReAct agent over GPT-4o-mini. The agent retrieves raw document chunks and performs a single LLM generation — avoiding the latency and token cost of a double-generation pass. The agent can also generate **inline interactive charts** (price history, multi-stock comparison, financial metrics) directly in the chat. Conversation history and charts survive page refreshes (backed by server-side memory) and are cleanly wiped on `docker compose down`. Users interact through a Streamlit chat UI. The stack is built on LangChain / LangGraph, FastAPI, ChromaDB, Elasticsearch, Plotly, and Streamlit, and ships with a built-in RAGAS evaluation dashboard.
...
- **backend-api**: FastAPI service hosting the LangGraph ReAct agent. Routes user questions to the RAG service, Yahoo Finance tools, inline chart generation, or direct answers, and exposes `POST /chat` for the web UI.
...
- **eval-dashboard**: Streamlit dashboard that runs four RAGAS metrics (Context Recall, Context Precision, Faithfulness, Factual Correctness) over a curated 22-question financial Q&A benchmark. Supports side-by-side hybrid vs. dense-only comparison with delta chips, per-chunk provenance badges, and per-mode CSV export.
...
- The LangGraph ReAct agent in `backend-api` decides per-turn whether to call the RAG service for document-grounded answers, the Yahoo Finance tools (`get_stock_price`, `get_financial_info`) for live market data, or to answer directly. When it calls the RAG service, it receives raw document chunks and performs a **single LLM generation** — the `api` service handles retrieval only, avoiding a redundant generation pass.
- Retrieval-augmented answers are generated using hybrid retrieval: dense vector search over ChromaDB and BM25 keyword search over Elasticsearch, fused with Reciprocal Rank Fusion (top 5 chunks). This improves retrieval of exact financial terms, figures, and dates compared to dense-only search.
- The agent can generate **interactive Plotly charts** inline in the chat. Ask for a price history ("Show me Apple's stock price over the last 90 days"), a multi-stock comparison ("Compare Tesla and Rivian over 6 months"), or a financial metrics breakdown ("Show Microsoft's financial metrics").
- **Conversation history** is stored server-side in the LangGraph `MemorySaver`. It survives page refreshes automatically and is cleanly wiped when the stack is restarted with `docker compose down`. Charts generated during a session are also restored on refresh.
- Retrieval and answer quality can be measured at any time from the `eval-dashboard` service. The dashboard supports selecting individual metrics, comparing hybrid vs. dense-only retrieval side-by-side, and downloading per-question results as CSV.
